...
namespace CMS_CP6FINAL.Controllers
{
    ...
    public class LabTestController : ControllerBase
    {
        ...
        public IActionResult DeleteLabTest(int id)
        {
            ...
            [HttpPost("v1")]
    public async Task<ActionResult<int>> InsertLabTestReturnId(LabTest labTest)
    {
        if (ModelState.IsValid)
        {
            var newLabTestId = await _service.PostLabTestReturnId(labTest);
            if (newLabTestId != 0) // Assuming 0 is an invalid ID
            {
                return Ok(newLabTestId);
            }
            else
            {
                return NotFound();
