...
public class LoginRepository : ILoginRepository
{
    ...
    //        "Orthopedics", "Gastroenterology", "Dermatology","HR","Administration","Laboratory","Pharmacy"
    //    };
    ...
    return await _context.Set<Staff>()
        .Include(s => s.Department) // Ensure we include the department data in the query
        .Where(s => eligibleDepartments.Contains(s.Department.DepartmentName) 
                    && !_context.UserRegistrations.Any(u => u.StaffId == s.StaffId)) // Correctly filter out users with credentials
        .ToListAsync();
}
